#!/bin/bash
# Runs AttNHP_train on taxi dataset.
...
CONFIG_FILE="run/AttNHP_train/taxi_config.yaml"
...
echo "Config: run/AttNHP_train/taxi_config.yaml"
...
" "run/AttNHP_train/taxi_config.yaml" "AttNHP_train" "taxi" "$MAIN_CSV" "$TIMESTAMP" > "$LOG_FILE" 2>&1
