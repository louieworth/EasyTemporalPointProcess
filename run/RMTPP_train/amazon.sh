#!/bin/bash
# Runs RMTPP_train on amazon dataset.
...
CONFIG_FILE="run/RMTPP_train/amazon_config.yaml"
...
echo "Config: run/RMTPP_train/amazon_config.yaml"
...
" "run/RMTPP_train/amazon_config.yaml" "RMTPP_train" "amazon" "$MAIN_CSV" "$TIMESTAMP" > "$LOG_FILE" 2>&1
