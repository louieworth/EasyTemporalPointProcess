#!/bin/bash
# Runs RMTPP_train on taxi dataset.
...
CONFIG_FILE="run/RMTPP_train/taxi_config.yaml"
...
echo "Config: run/RMTPP_train/taxi_config.yaml"
...
" "run/RMTPP_train/taxi_config.yaml" "RMTPP_train" "taxi" "$MAIN_CSV" "$TIMESTAMP" > "$LOG_FILE" 2>&1
