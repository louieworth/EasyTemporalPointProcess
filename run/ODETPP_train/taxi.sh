#!/bin/bash
# Runs ODETPP_train on taxi dataset.
...
CONFIG_FILE="run/ODETPP_train/taxi_config.yaml"
...
echo "Config: run/ODETPP_train/taxi_config.yaml"
...
" "run/ODETPP_train/taxi_config.yaml" "ODETPP_train" "taxi" "$MAIN_CSV" "$TIMESTAMP" > "$LOG_FILE" 2>&1
